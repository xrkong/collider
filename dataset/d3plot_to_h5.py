...
--------
  ...
  Step 3  – Per-frame extraction  (direct from d3plot, no manual math)
      For each kept frame:
        ...
        • velocity      (xyz)         node_velocity  (or numerical ∂pos/∂t)
        • acceleration  (xyz)         node_acceleration (or numerical ∂²pos/∂t²)
...
-----------
  ...
  /states/
      ...
      velocity            (T, N, 3)   float32  [mm/s]
      acceleration        (T, N, 3)   float32  [mm/s²]
...
# ── Numerical velocity / acceleration fallbacks ────────────────────────────────

def _numerical_velocity(positions: np.ndarray, times: np.ndarray) -> np.ndarray:
    """∂pos/∂t via np.gradient (non-uniform spacing).  Returns (T, N, 3) float32."""
    if len(times) < 2:
        return np.zeros_like(positions, dtype=np.float32)
    return np.gradient(positions.astype(np.float64), times, axis=0, edge_order=2).astype(np.float32)


def _numerical_acceleration(positions: np.ndarray, times: np.ndarray) -> np.ndarray:
    """∂²pos/∂t² via two np.gradient calls.  Returns (T, N, 3) float32."""
    if len(times) < 3:
        return np.zeros_like(positions, dtype=np.float32)
    vel = np.gradient(positions.astype(np.float64), times, axis=0, edge_order=2)
    return np.gradient(vel, times, axis=0, edge_order=2).astype(np.float32)
...
def main() -> None:
    ...
    # We always need positions in memory to support fallback vel/acc computation.
    positions_all = np.empty((n_frames, n_nodes, 3), dtype=np.float32)
    times_all     = np.empty(n_frames,               dtype=np.float64)

    # These are set to True/False on first file open (assumed consistent across files)
    has_vel: bool | None = None
    has_acc: bool | None = None
            ...
            if current_name != fname:
                ...
                d3 = D3plot(
                    ...
                    state_array_filter=[
                        ...
                        ArrayType.node_velocity,       # read directly if present
                        ArrayType.node_acceleration,   # read directly if present
                        ArrayType.element_solid_stress,
                        ArrayType.element_shell_stress,
                    ],
                )

                # detect once which direct arrays are available
                if has_vel is None:
                    has_vel = d3.arrays.get(ArrayType.node_velocity)     is not None
                    has_acc = d3.arrays.get(ArrayType.node_acceleration) is not None
                    print(f"    node_velocity     in d3plot: {has_vel}")
                    print(f"    node_acceleration in d3plot: {has_acc}")
    ...
    print(f"Velocity src   : {'d3plot (direct)' if has_vel else 'numerical ∂pos/∂t'}")
    print(f"Accel src      : {'d3plot (direct)' if has_acc else 'numerical ∂²pos/∂t²'}")
